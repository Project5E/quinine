...
class RxTestQuinineLoadingCache {
    ...
    fun testGetOne() {
        ...
        runBlocking { assertEquals(Common.primes[index - 1], cache.get(index).await()) }
        ...
        runBlocking {
            ...
            val jobs = List(cores - 1) {
                async(Dispatchers.Default) {
                    ...
                    for (i in 1..2000) {
                        val index = random.nextInt(1, 1000)
                        assertEquals(Common.primes[index - 1], cache.get(index).await())
                    }
                }
            }
